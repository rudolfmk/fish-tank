...
export function RedFlagAlerts({segments,live}:{segments:Segment[];live:boolean}){
  ...
  if(!active.length)return <div className="flex items-center gap-2 rounded-xl border border-line bg-white px-4 py-3 text-xs text-muted"><ShieldCheckIcon className={`h-4 w-4 ${scanning?"animate-pulse text-amber-600":reviewed.length?"text-red-600":"text-emerald-600"}`}/>{scanning?"Red-flag safety scan running…":reviewed.length?`${reviewed.length} red-flag alert${reviewed.length===1?"":"s"} acknowledged by clinician: ${reviewed.map(alert=>alert.condition).join(", ")}.`:error?`Red-flag safety scan unavailable: ${error}`:scannedLines!==null?`No red-flag patterns detected in ${scannedLines} transcript line${scannedLines===1?"":"s"}${live?" so far":""}.`:"Red-flag safety scan pending…"}{reviewed.length>0&&<button onClick={()=>setAcknowledged(new Set())} className="ml-auto font-bold text-red-700 underline">Show</button>}</div>;
